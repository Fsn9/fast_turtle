...
#include <string.h>
// Services
#include "fast_turtle/ResetArena_two.h"
...
void init_graphics_and_data(){
    ...
    for(i = 0; i < ft->get_world()->get_n_simple_drones(); i++)
    {
        ...
        for(int sw = 0; sw < 4; sw++)
        {
            ...
            sd_support_marker.color.r = 0.0; //Drones[i].rgb[0];//0.0;
            sd_support_marker.color.g = 0.0; //Drones[i].rgb[1]; //0.0;
            sd_support_marker.color.b = 0.0; //Drones[i].rgb[2]; //0.0;
...
bool reset_arena(fast_turtle::ResetArena_two::Request &req, fast_turtle::ResetArena_two::Response& res)
{
    ft->reset_robots();
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber sub_sd2 = nh.subscribe("cmd_vel_sd2_two"+ user_id, 1000, listen_cmd_vel_sd2);
    ros::Subscriber sub_sd3 = nh.subscribe("cmd_vel_sd3_two"+ user_id, 1000, listen_cmd_vel_sd3);
    ros::Subscriber sub_sd4 = nh.subscribe("cmd_vel_sd4_two"+ user_id, 1000, listen_cmd_vel_sd4);
